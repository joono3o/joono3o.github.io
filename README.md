...
로지스틱 회귀 모델을 사용한 결과, 정확도가 약 77%정도로 결과가 나왔다. 이는 꽤나 높은 연관성을 보여준다고 할 수 있다. 특히, modern stat인 avg. exit speed(평균 타구 속도)가 더 높을수록 소포모어 징크스를 겪을 확률이 낮을 것으로 예측되는 유의미한 결과를 도출할 수 있었다. 이 결과가 시사하는 바는, 클래식 스탯(타율, 홈런)는 바람, 야수의 판단 실수 등과 같은 운을 내포하고 있기에, 그 선수의 실력을 완전 정확하게 파악하기는 힘들다는 것이다. 거기에 더해서, 1년차때는 보통 분석이 덜 되어있어서 그 선수의 성적이 실력보다 잘 나올 수도 있다. 약점은 감추고, 강점을 살릴 수 있는 기회가 많이 찾아올 수도 있는데, 2년차때는 분석을 당하며 그런 기회가 줄어들고 본 실력이 드러나는 것이다.
